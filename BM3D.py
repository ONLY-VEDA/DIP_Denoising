...
def eval():
    ...
    parser.add_argument('--eval_set', dest='eval_set', default='./data/BSD68/test', help='dataset for eval in training')
    ...
    for i, data in enumerate(sidd_db):
        inputs, labels = data['noise'][0]*255, data['clean'][0]*255
        out = bm3d(inputs)

        psnr_pred = cal_psnr(labels, out)
...
def infer():
    noise_img = cv2.imread('test.png')
    noise_img = cv2.cvtColor(noise_img, cv2.COLOR_BGR2GRAY)

    out = bm3d(noise_img)

    cv2.imwrite("BM3D.png", out)
